...
export default class RunSuiteAction extends LightningElement {
    @api recordId;
    ...
    @wire(getScriptsForSuite, { testSuiteId: '$recordId' })
    wiredScripts({ data, error }) {
        if (data) {
            this.scripts   = data;
            ...
            this.showToast('Error loading scripts',
                error.body?.message ?? 'Could not retrieve scripts for this suite.', 'error');
        }
    }
    ...
    async handleRunSelected() {
        ...
        this.showToast(
            'Runs Queued',
            `${passed} run(s) started${failed > 0 ? `, ${failed} failed to start` : ''}.`,
